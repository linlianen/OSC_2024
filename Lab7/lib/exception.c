...
void sys_mkdir(tf* trap_frame){
  int ret = vfs_mkdir((char*)trap_frame->x0);
  trap_frame->x0 = ret;
}

void sys_open(tf* trap_frame){
  struct file* fd = vfs_open((char*)trap_frame->x0, trap_frame->x1);
  
  int fd_num = thread_register_fd(fd);
  trap_frame->x0 = fd_num;
}
