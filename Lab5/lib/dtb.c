      ...
      case FDT_PROP:
        ...
        if (callback != NULL)
        {
          ...
          if(reserved_memory ==1){
            if(!my_strcmp(propName, "#address-cells")){

              address = (char*)(uintptr_t)get_be_int((void*)propVal);   
            }else if(!my_strcmp(propName, "#size-cells")){

              size = (char*)(uintptr_t)get_be_int(size);   
            }
