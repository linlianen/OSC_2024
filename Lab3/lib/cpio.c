...
static void align(void *size, size_t s);
/* char to Int */
...
static int cpio_strncmp(const char *s1, const char *s2, unsigned long long n) {
  unsigned char c1 = '\0';
  unsigned char c2 = '\0';
  if (n >= 4) {
...
/* Parse Cpio header */
int cpio_parse_header(struct cpio_newc_header *archive, const char **filename,
 ...
 * */
{
  ...
  *filename = ((char *)archive) + sizeof(struct cpio_newc_header);
  ...
  if (cpio_strncmp(*filename, CPIO_FOOTER_MAGIC, sizeof(CPIO_FOOTER_MAGIC)) ==
      0)
    return 1;
  ...
  *data =
      (void *)align_up(((unsigned long)archive) +
                           sizeof(struct cpio_newc_header) + filename_length,
                       CPIO_ALIGNMENT);
  *next = (struct cpio_newc_header *)align_up(((unsigned long)*data) + filesize,
                                              CPIO_ALIGNMENT);
  if (_filesize) {
    *_filesize = filesize;
  }
...
void *cpio_get_file(void *archive, const char *name, uint64_t *size) {
  struct cpio_newc_header *header = (struct cpio_newc_header *)archive;
  ...
  while (1) {
    ...
    int error =
        cpio_parse_header(header, &current_filename, size, &result, &next);
...
char *findFile(char *name, char *cpio_addr) {
  ...
  while (my_strcmp((char *)(addr + sizeof(struct cpio_newc_header)),
                   "TRAILER!!!") != 0) {
    if ((my_strcmp((char *)(addr + sizeof(struct cpio_newc_header)), name) ==
         0)) {
      return addr;
    }
    ...
    unsigned int headerPathname_size =
        sizeof(struct cpio_newc_header) + pathname_size;
...
void cpio_load_prog(char *filename, const char *cpio_buf) {
  ...
  if (prog_addr) {
    ...
    //  set the user program’s stack pointer to a proper position by setting
    //  sp_el0. issue eret to return to the user code. asm volatile("mov x0,
    //  0x340  \n");
    /*
